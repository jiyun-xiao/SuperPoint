...
def vgg_block(inputs, filters, kernel_size, name, activate, data_format, training=False,
...
def vgg_backbone(inputs, **config):
    ...
    with tf.variable_scope('vgg', reuse=tf.AUTO_REUSE):
        x = vgg_block(inputs, 64, 3, 'conv1_1', True, **params_conv)
        x = vgg_block(x, 64, 3, 'conv1_2', True, **params_conv)
        x = tfl.max_pooling2d(x, 2, 2, name='pool1', **params_pool)

        x = vgg_block(x, 64, 3, 'conv2_1', True, **params_conv)
        x = vgg_block(x, 64, 3, 'conv2_2', True, **params_conv)
        x = tfl.max_pooling2d(x, 2, 2, name='pool2', **params_pool)

        x = vgg_block(x, 128, 3, 'conv3_1', True, **params_conv)
        x = vgg_block(x, 128, 3, 'conv3_2', True, **params_conv)
        x = tfl.max_pooling2d(x, 2, 2, name='pool3', **params_pool)

        x = vgg_block(x, 128, 3, 'conv4_1', True, **params_conv)
        x = vgg_block(x, 128, 3, 'conv4_2', True, **params_conv)
